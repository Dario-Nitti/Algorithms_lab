...
public class UndirectedWeightedTree<Vertex extends Comparable<? super Vertex>> extends UndirectedNetwork<Vertex>
{
    ...
    public boolean equals (Object obj)
    {
        if (!(obj instanceof UndirectedWeightedTree<?>))
            return false;
        UndirectedWeightedTree<?> other = (UndirectedWeightedTree<?>)obj;
    ...
    public boolean addEdge (Vertex v1, Vertex v2, double weight)
    {
        ...
        if (root == null)
            root = v1;
        okToAddVertex = true;
        boolean result = super.addEdge (v1, v2, weight);
        okToAddVertex = false;
        return result;
    } // method addEdge
    ...
    public boolean removeVertex (Vertex v)
    {
        ...
        if (size > 1) // internal node
            return false;
        // size = 0: v must be the root (singleton)
        if (size == 0) root = null;
        // size = 1: v is either the root or a leaf
        if (size == 1 && root.equals(v))
            //root has only one child and therefore move root to the its child
            root = adjacencyMap.get (root).firstKey();
        // v is a leaf
